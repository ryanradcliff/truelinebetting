...
function computePayout(){
  ...
  if(state.slip.length === 0){ $('#payoutValue').textContent = toCurrency(0); return; }/* TrueLineBetting v4 — AI Picks, win rate & rationale */
const $ = (sel, el=document) => el.querySelector(sel);
const $$ = (sel, el=document) => Array.from(el.querySelectorAll(sel));
...
/* --- FIX: run router whenever the hash changes --- */
